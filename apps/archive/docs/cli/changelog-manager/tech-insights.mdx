...
- Should get the current branch name and default branch name (main/master).
...
- Should create the **change file**.
  ...
  - If there were no new commits comparing with the default branch (`main` or `master`), should print the following message and exit with status code 0: `Error: No new commits found in the current branch. Please make sure you have committed your changes and your branch is ahead of origin/main`
...
- Should fetch the latest changes from the remote repository.
- Should get the current branch name and default branch name (main/master).
- Should verify that there are new commits in the current branch comparing with the default branch.
...
- Should verify that change files exist and are valid, the same as `rune verify` command.
- Should create changelog files if it doesn't exist.
- Should get the date from change files name.
