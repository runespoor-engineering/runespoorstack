...
- If the issue link pattern is provided, should ask to enter the issue id. Message: `Enter the issue id:`.
...
- Should modify changelog file with:
  ...
  - Author name
  - Issue Link (if `--issueLinkPattern` was provided to `rune change` command)
...
[Issue Link](<issue-link>) | Author: **GitUserName**
